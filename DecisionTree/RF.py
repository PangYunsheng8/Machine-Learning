...
class RandomForest:
    def __init__(self, n_tress, max_depth, n_feats, classifier=True, criterion="entropy"):
        """
    ...
    def predict(self, X):
        ...
        Returns
        -------
        y_pred: `ndarray <numpy.ndarray>` of shape `(N,)`
            Model predictions for each entry in `X`.
        """
        tree_preds = np.array([[t._traverse(x, t.root) for x in X] for t in self.tress])
